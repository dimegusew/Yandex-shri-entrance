...
class MainBodyEvent extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      timeStart: this.props.isEditedPage
        ? converterFromServerTime(this.props.eventToEdit.dateStart).time
        : this.props.timeToNewEvent.start,
      timeEnd: this.props.isEditedPage
        ? converterFromServerTime(this.props.eventToEdit.dateEnd).time
      ...
      timeStartValid:
        this.props.timeToNewEvent.start || this.props.isEditedPage
          ...
          : false,
      timeEndValid:
        this.props.timeToNewEvent.end || this.props.isEditedPage ? true : false,
      dateValid:
        this.props.dateToNewEvent || this.props.isEditedPage ? true : false,
      choosedRoom: "",
  ...
  choosedUser = user => {
    ...
    let addedUsers = this.props.users
      ? this.props.users.filter(el => choosedUsers.indexOf(el.id) != -1)
      : "";
    this.setState({
  ...
  onInputHandler = change => {
    switch (change.target.id) {
      ...
      case "users":
        var viewedUsers;
        viewedUsers = this.props.users.filter(
          el => el.login.indexOf(change.target.value) != -1
        );
        this.setState({
  ...
  render() {
    ...
    let TimeStartValid = this.state.timeStart
      ? this.state.timeStart.length === 5
      : false;
    let TimeEndValid = this.state.timeEnd
      ? this.state.timeEnd.length === 5
      : false;
    let timeValid = TimeStartValid && TimeEndValid;
    let titleValid = this.state.EventTitle;
    let dateValid = this.state.date;
    let formValid =
      dateValid && titleValid && timeValid && !!this.state.choosedRoom;
      ...
      <div>
            ...
            <div className="event-edit">
              <div className="left-side">
                ...
                <div className="date-time-input">
                  <InputField
                    title={pageWidth < 425 ? "Дата и время" : "Дата"}
                    placeholder={"Дата"}
                ...
                <div className={"user-choose"}>
                  <InputField
                    ...
                    width={inputfieldWidth}
                    img={
                      this.state.isUsersListOpen ? arrowRotate : arrowRotateBack
                    }
                ...
                {timeValid ? (
                  <RecomendedRooms
                    ...
                    time={{
                      start:
                        this.state.date +
                        ("T" + this.state.timeStart + ":00.981Z"),
                      end:
                        this.state.date +
                        ("T" + this.state.timeEnd + ":00.981Z")
                    }}
                  />
                ) : (
                  ""
                ...
                <BottomBar
                  ...
                  dataToServer={{
                    dateStart:
                      ...
                      ("T" + this.state.timeStart + ":00.981Z"),
                    dateEnd:
                      this.state.date + ("T" + this.state.timeEnd + ":00.981Z"),
        ...
        {pageWidth > 425 ? (
          <BottomBar
            ...
            dataToServer={{
              dateStart:
                this.state.date + ("T" + this.state.timeStart + ":00.981Z"),
              dateEnd:
                this.state.date + ("T" + this.state.timeEnd + ":00.981Z"),
              room: this.state.choosedRoom,
              users: this.state.addedUsers,
