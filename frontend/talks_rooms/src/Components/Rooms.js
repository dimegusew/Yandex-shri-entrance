...
const Room = ({ el, ...props }) => {
  ...
  let style =
    el.id === props.choosedId
      ? { backgroundColor: " #007DFF", color: "white" }
      : {};
  return (
    <div className="Room" id={el.id} style={style} onClick={props.onClick}>
      ...
      <div onDeleteClick={props.onDeleteClick}>
        {el.id === props.choosedId && "x"}
      </div>
    </div>
  );
};
...
const Rooms = ({ rooms, ...props }) => {
  return (
    <div className="Rooms">{rooms.map(el => <Room el={el} {...props} />)}</div>
  );
};
